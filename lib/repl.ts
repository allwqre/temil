...
import { AssertionError, assert_args, assert_args_greater, assert_expression, assert_literal } from './Assert.js';
...
const assert_num: (value: unknown, scope: string) => asserts value is number = (value, scope) => {
	if (typeof value !== 'number') throw new AssertionError(scope, `Expected ${value} to be type of number.`);
};
...
const assert_str: (value: unknown, scope: string) => asserts value is string = (value, scope) => {
	if (typeof value !== 'string') throw new AssertionError(scope, `Expected ${value} to be type of string.`);
};
